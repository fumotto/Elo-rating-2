...
import { withRetries } from "./apiClient";
...
// Ensure server-side profile exists for the authenticated user.
export async function ensureUserProfile(): Promise<UserProfile | null> {
  const { data, error } = await supabase.rpc("ensure_user_profile");
  if (error) {
    // don't throw to avoid breaking sign-in flow; caller can handle null
    console.warn("ensure_user_profile RPC failed:", error.message ?? error);
    return null;
  }
  return (data ?? null) as UserProfile | null;
}
...
export async function fetchAppSettings(): Promise<AppSettings> {
  return await withRetries(async () => {
    ...
    return { k_factor: value.value ?? 32, k_editable: value.editable ?? false };
  });
}
...
export async function deletePlayer(playerId: string): Promise<void> {
  await withRetries(async () => {
    const { data, error } = await supabase.rpc("delete_player_safe", {
